 ...
 * @Date 2019/11/13 10:27 下午
 * @Created By guanbao.zhou
 */
public class GcSurvivorDemo {

  public static void main(String[] args) throws InterruptedException {
    // 这两个对象不会被回收, 用于在s0和s1不断来回拷贝增加age直到达到MaxTenuringThreshold晋升到old
    ...
    // 即5M*60%=3M(Desired survivor size)，说明: 5M为S区的大小，60%为TargetSurvivorRatio参数指定，
    // 如下三个对象分配后就能够达到Desired survivor size
    ...
    // 而不需要等MaxTenuringThreshold这么多次, 此次ygc后, s0(from)和s1(to)空间中对象再次被清空, 对象全部晋升到old
    youngGc(1);
    Thread.sleep(3000);
